...
test('SDIRA draft can be created without selecting a payment method', () => {
  ...
  assert.equal(result.accountType, 'sdira')
  assert.equal(result.paymentMethod, null)
})

test('SDIRA submissions still require wire payment method', () => {
  assert.throws(
    () => validatePaymentMethod('ach', 25000, 'sdira'),
    /SDIRA accounts must use wire transfer/
  )
})
...
test('Draft payment helper prefers wire in required scenarios', () => {
  assert.equal(determineDraftPaymentMethod('sdira', 1000), 'wire')

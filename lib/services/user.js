...
class UserService extends ApiService {
  ...
  /**
   * Manually add bank account with real-time ACHQ verification
   * Account is verified instantly and ready to use on success
   */
  async createManualPaymentMethod({ routingNumber, accountNumber, accountType = 'checking', accountHolderType = 'Personal' }, idempotencyKey) {
   ...
   * Returns documents uploaded by admin for this user
   */
  async getDocuments() {
    try {
      ...
      return {
        success: true,
        documents: data.documents || data.items || data || []
      }
    } catch (error) {
      console.error('[UserService] Failed to get documents:', error)
      return {
        success: false,
        documents: [],
        error: error.message
   ...
   * Backend endpoint: GET /api/documents/{document_id}
  ...
  async getDocument(documentId) {
    ...
    try {
      ...
      console.error('[UserService] Download document failed:', error)
      return {
        success: false,
        error: error.message
      }
  ...
  async uploadDocument(file) {
    ...
    // Do NOT set Content-Type - browser must set boundary for multipart/form-data
    ...
    try {
      ...
      console.error('[UserService] Upload document failed:', error)
      return {
        success: false,

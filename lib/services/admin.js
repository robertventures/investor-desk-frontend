...
class AdminService extends ApiService {
  ...
  // =====================================================================
  // 1099-INT SUBMISSIONS
  // =====================================================================

  /**
   * Get 1099-INT submissions (admin only)
   * @param {Object} params - Filter parameters
   * @param {string} params.user_id - Filter by user ID
   * @param {string} params.status - Filter by status (pending, invalid, valid, submitted, transmitted, failed, success, finished)
   * @param {string} params.tax_year - Filter by tax year (e.g., "2024")
   * @param {number} params.page - Page number (default: 1)
   * @param {number} params.size - Page size (default: 50)
   */
  async get1099INTSubmissions(params = {}) {
    try {
      const queryParams = new URLSearchParams()
      ...
      if (params.status) queryParams.append('status', params.status)
      if (params.tax_year) queryParams.append('tax_year', params.tax_year)
      
      const data = await this.request(`/api/admin/1099int-submissions?${queryParams.toString()}`, {
        method: 'GET',
      })
      ...
      logger.error('[AdminService] Get 1099-INT submissions failed:', error)
      return {
        success: false,
        submissions: [],
        total: 0,
        error: error.message || 'Failed to get 1099-INT submissions'
      }
    }
  }

  /**
   * Generate 1099-INT for a single user (admin only)
   * @param {string} userId - User ID to generate 1099-INT for
   * @param {string} taxYear - Tax year (e.g., "2024")
   */
  async generate1099INTForUser(userId, taxYear) {
    const numericId = userId.toString().replace(/\D/g, '')
    
    try {
      logger.info(`[AdminService] Generating 1099-INT for user ${numericId}, tax year ${taxYear}`)
      const data = await this.request(`/api/admin/1099int-submissions/users/${numericId}/generate`, {
        method: 'POST',
      ...
      return {
        success: true,
        userId: data.userId,
        status: data.status, // 'created', 'skipped', or 'failed'
        reason: data.reason || null,
        submission: data.submission || null,
        ...data
      }
    } catch (error) {
      logger.error('[AdminService] Generate 1099-INT failed:', error)
      const errorMessage = error.responseData?.detail 
        || error.responseData?.error 
        || error.responseData?.message 
        || error.message 
        || 'Failed to generate 1099-INT'
      return {
        success: false,
        status: 'failed',
        error: errorMessage,
  ...
  /**
   * Sync 1099-INT submission status with TaxBandits (admin only)
   * @param {number} submissionId - Submission ID to sync
   */
  async sync1099INTStatus(submissionId) {
    try {
      logger.info(`[AdminService] Syncing 1099-INT status for submission ${submissionId}`)
      const data = await this.request(`/api/admin/1099int-submissions/${submissionId}/sync-status`, {
        method: 'POST',
      })
      ...
      logger.error('[AdminService] Sync 1099-INT status failed:', error)
      return {
        success: false,
        error: error.message || 'Failed to sync 1099-INT status'
      }
    }
  }

  /**
   * Request PDF generation for a 1099-INT submission (admin only)
   * @param {number} submissionId - Submission ID to request PDF for
   */
  async requestPdf1099INT(submissionId) {
    try {
      logger.info(`[AdminService] Requesting PDF for 1099-INT submission ${submissionId}`)
      const data = await this.request(`/api/admin/1099int-submissions/${submissionId}/request-pdf`, {
        method: 'POST',
      })
      ...
      logger.error('[AdminService] Request 1099-INT PDF failed:', error)
      return {
        success: false,
        error: error.message || 'Failed to request 1099-INT PDF'
      }
    }
  }

  /**
   * Manually submit a 1099-INT form (admin only)
   * Used for forms in 'valid' status that need manual submission
   * @param {number} submissionId - Submission ID to submit
   */
  async submit1099INT(submissionId) {
    try {
      logger.info(`[AdminService] Submitting 1099-INT submission ${submissionId}`)
      const data = await this.request(`/api/admin/1099int-submissions/${submissionId}/submit`, {
        method: 'POST',
      })
      ...
      logger.error('[AdminService] Submit 1099-INT failed:', error)
      return {
        success: false,
        error: error.message || 'Failed to submit 1099-INT'
      }
    }
  }

  /**
   * Manually transmit a 1099-INT form to the IRS (admin only)
   * Used for forms in 'submitted' status that need manual transmission
   * @param {number} submissionId - Submission ID to transmit
   */
  async transmit1099INT(submissionId) {
    try {
      logger.info(`[AdminService] Transmitting 1099-INT submission ${submissionId}`)
      const data = await this.request(`/api/admin/1099int-submissions/${submissionId}/transmit`, {
        method: 'POST',
      })
      ...
      logger.error('[AdminService] Transmit 1099-INT failed:', error)
      return {
        success: false,
        error: error.message || 'Failed to transmit 1099-INT'
      }
    }
  }

  /**
   * Update a 1099-INT submission (admin only)
   * Only allowed for submissions in 'pending' or 'invalid' status
   * @param {number} submissionId - Submission ID to update
   * @param {Object} data - Fields to update
   */
  async update1099INTSubmission(submissionId, updateData) {
    try {
      logger.info(`[AdminService] Updating 1099-INT submission ${submissionId}`)
      const data = await this.request(`/api/admin/1099int-submissions/${submissionId}`, {
        method: 'PATCH',
      ...
      logger.error('[AdminService] Update 1099-INT submission failed:', error)
      return {
        success: false,
        error: error.message || 'Failed to update 1099-INT submission'
      }
    }
  }
}

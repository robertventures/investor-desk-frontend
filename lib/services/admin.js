...
class AdminService extends ApiService {
  ...
  /**
   * Get pending payouts (distribution transactions with pending status)
   */
  async getPendingPayouts(params = {}) {
    const queryParams = new URLSearchParams()
    
    // Filter for pending distribution transactions (payouts)
    ...
    const pendingPayouts = items.map(tx => ({
      id: tx.id,
      amount: tx.amount,
      status: tx.status,
  ...
  async uploadUserDocument(userId, file) {
    ...
    // Custom request logic for multipart/form-data
    // We can't use this.request() because it adds Content-Type: application/json
    // and for FormData, the browser must set the boundary automatically
    
    const url = this.buildUrl(`/api/admin/users/${numericId}/documents`)
    ...
    try {
      ...
      logger.error('[AdminService] Upload document failed:', error)
      return {
        success: false,
  ...
  async getUserDocuments(userId) {
    const numericId = userId.toString().replace(/\D/g, '')
    try {
      const data = await this.request(`/api/admin/users/${numericId}/documents`, {
        method: 'GET'
      })
      ...
      logger.error('[AdminService] Get documents failed:', error)
      return {
        success: false,
        documents: [],
        error: error.message
  ...
  /**
   * Get/Download user document (admin only)
   */
  async getUserDocument(userId, documentId) {
    const numericId = userId.toString().replace(/\D/g, '')
    const url = this.buildUrl(`/api/admin/users/${numericId}/documents/${documentId}`)
    ...
    try {
      ...
      logger.error('[AdminService] Download document failed:', error)
      return {
        success: false,
        error: error.message
      }
  ...
  async deleteUserDocument(userId, documentId) {
    const numericId = userId.toString().replace(/\D/g, '')
    try {
      await this.request(`/api/admin/users/${numericId}/documents/${documentId}`, {
        method: 'DELETE'
      })
      ...
      logger.error('[AdminService] Delete document failed:', error)
      return {
        success: false,
        error: error.message
      }

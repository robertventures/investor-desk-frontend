  ...
  async request(endpoint, options = {}) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Time machine is optional - 404 means it's not available in this environment
        const isOptionalFeatureNotFound = response.status === 404 && endpoint.includes('/api/admin/time-machine')

        if (!isProfileLocked && !isExpectedConflict && !isOptionalFeatureNotFound) {

...
export function validateAccountType(type) {
  const validTypes = ['individual', 'joint', 'entity', 'sdira']
 ...
 */
export function validatePaymentMethod(method, amount, accountType, options = {}) {
  const { required = true } = options
  const normalizedMethod = typeof method === 'string' ? method.toLowerCase().trim() : method
  const isMissing = normalizedMethod === undefined || normalizedMethod === null || normalizedMethod === ''
  ...
  if (normalizedMethod === 'ach' && amount > 100000) {
    throw new ValidationError(
  ...
  // Business rule: SDIRA must use wire
  if (accountType === 'sdira' && normalizedMethod !== 'wire') {
    throw new ValidationError('SDIRA accounts must use wire transfer', 'paymentMethod')
  }
...
export function validateInvestmentData(data) {
  ...
  // Business rule: SDIRA cannot have monthly payouts
  if (validated.accountType === 'sdira' && validated.paymentFrequency === 'monthly') {
    throw new ValidationError(
      'SDIRA accounts can only use compounding payment frequency',
      'paymentFrequency'
    )
  }
 ...
 */
export const normalizePhoneForDB = (value = '') => {
 ...
 */
export const isValidUSPhoneDigits = (raw = '') => {
